...
def trend_sarimax(data, city, steps=12):
    ...
    city_M_sarimax = sm.tsa.SARIMAX(endog=city_ts_M, order=[1,1,0], seasonal_order=[1,1,1,12], simple_differencing=True).fit(maxiter=200, method='lbfgs', disp=False)
...
def trend_ses(data, city, steps=12):
    city_ts = data[data['city'] == city]['temperature']
    city_ts_M = pd.DataFrame(city_ts.resample('ME').mean())
    ES = exponential_smoothing.ExponentialSmoothing
    config = {"trend": True}
    stlf = sm.tsa.STLForecast(city_ts_M, ES, model_kwargs=config)
    res = stlf.fit(maxiter=200, method='bfgs', disp=False)
    forecast = res.forecast(steps)
    city_ts_final = pd.DataFrame(city_ts_M).merge(
        pd.DataFrame(forecast).rename(columns={'predicted_mean': 'forecast'}),
        how='outer', left_index=True, right_index=True)
    future_mean = city_ts_final['forecast'].iloc[-steps:].mean()
...
def city_data_processing(data, city, window=30, steps=12):
    ...
    #trend = trend_ses(data, city, steps)
